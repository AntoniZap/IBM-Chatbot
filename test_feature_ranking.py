...
def test_addRating1():
    ...
    feature_ranking.addRating1(User_id, query, llm, rating)
    # Check if the rating was added
    print(feature_ranking.getUserRating(User_id, llm))
    assert feature_ranking.getUserRating(User_id, llm) == rating
...
def test_addRating2():
    ...
    feature_ranking.addRating2(User_id, query, llm1, rating1, llm2, rating2)

    # Check if the ratings were added
    assert feature_ranking.getUserRating(User_id, llm1) == rating1
    assert feature_ranking.getUserRating(User_id, llm2) == rating2
...
def test_addRating3():
    ...
    feature_ranking.addRating3(User_id, query, llm1, rating1,
                              llm2, rating2, llm3, rating3)

    # Check if the ratings were added
    assert feature_ranking.getUserRating(User_id, llm1) == rating1
    assert feature_ranking.getUserRating(User_id, llm2) == rating2
    assert feature_ranking.getUserRating(User_id, llm3) == rating3
...
def test_getAverageRating():
    ...
    # Add all three test ratings to the database
    feature_ranking.addRating1(User_id1, query, llm, rating1)
    feature_ranking.addRating1(User_id2, query, llm, rating2)
    feature_ranking.addRating1(User_id3, query, llm, rating3)

    assert feature_ranking.getAverageRating(
